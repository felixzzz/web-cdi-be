<template>
    <div class="mt-10 mb-2 flex items-center justify-between flex-col lg:flex-row">
        <a
            class="text-xs lg:text-base cursor-pointer px-6 py-2 rounded-full whitespace-nowrap flex items-center gap-2 text-blue-base border border-blue-base"
        >
            Filter
            <img :src="asset('assets/frontend/icons/ic_slidershorizontal.svg')" alt="">
        </a>

        <div class="w-full lg:w-[264px] rounded-full border border-neutral-7 px-4 py-2 flex items-center gap-1">
    ...
    <section v-if="!paginate.state.loading">
        <div class="py-8 border-b border-b-neutral-5 flex lg:items-center justify-between flex-col lg:flex-row gap-y-2 lg:gap-y-0" v-for="(item, i) in paginate.state.items" :key="i">
            ...
            <div class="flex lg:items-center gap-8 w-full lg:w-fit">
                <a :href="addFilePreview('press-release', item.ulid, 'default', item.name_slug)" class="flex items-center gap-2 text-blue-base font-medium" target="_blank">
                    <img :src="asset('assets/frontend/icons/ic_eye.svg')" alt=""> {{ $t('View Report') }}
                </a>
                <a :href="addFileDownload('press-release', item.ulid, 'en', item.name_slug_en)" class="flex items-center gap-2 text-blue-base font-medium" target="_blank" v-if="item.file_en">
                    <img :src="asset('assets/frontend/icons/ic_download_file.svg')" alt=""> {{ $t('Download-EN') }}
                </a>
                <a :href="addFileDownload('press-release', item.ulid, 'id', item.name_slug_id)" class="flex items-center gap-2 text-blue-base font-medium" target="_blank" v-if="item.file_id">
...
<script setup lang="ts">
    ...
    const paginate = usePaginate<PressRelease>({
        route: route("api.press-releases.list"),
