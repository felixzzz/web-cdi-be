<template>
    <div
        ...
        :style="{
            'backgroundImage': `url(${content?.investor_share_banner?.file_url})`
        }"
    >
            ...
            <div class="text-white grid lg:grid-cols-2 gap-2 relative z-[1] items-end">
                <div>
                    <h1 class="text-2xl leading-6  lg:text-[52px] lg:leading-[60px] font-medium max-w-2xl" id="home_banner_title">
                        {{ content?.investor_share_banner?.title }}
                    </h1>
                    <div class="content max-w-2xl text-xs lg:text-base !text-neutral-4" v-html="content?.investor_share_banner?.content"></div>
                </div>
