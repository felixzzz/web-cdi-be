<template>
    <div class="flex aspect-video">
        <div
            ...
            class="relative w-[25%] bg-cover bg-center bg-no-repeat text-white cursor-pointer transition-all duration-300 ease-in-out flex-grow"
        ...
        >
            ...
            <div class="absolute inset-0 flex flex-col justify-end px-5 lg:px-10 pb-5 lg:pb-10 pt-10 text-white z-10">
                ...
                <div
                    ...
                    :class="{ 'max-h-[500px] opacity-100 mt-2': hoverIndex === index }"
                >
                    <div class="content !text-white !text-sm !font-light" v-html="item.description"></div>
                    <div class="flex mt-8 gap-2">
                        <div
                            v-for="(company, companyIndex) in item.companies" :key="companyIndex"
                            class="px-[15px] py-[6px] rounded-full border border-white text-sm flex items-center gap-2"
                        >
                            {{ company }}
...
<script setup lang="ts">
    ...
    const data = ref([
        ...
        },
        {
            ...
            `,
            companies: ['CLEAN WATER', 'DEMIN WATER', 'DEMIN WATER']
        },
        {
