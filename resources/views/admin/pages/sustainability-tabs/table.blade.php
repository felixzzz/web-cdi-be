...
@section('content')
    <x-layouts.search :route-add="route('admin.sustainability-tabs.create', ['category' => $category])" :show-add="true" />
                        ...
                        <x-portal::table.cell class="font-medium text-right">
                            <x-portal::dropdown-menu class="flex justify-end">
                                ...
                                <x-portal::dropdown-menu.content class="w-fit" align="end">
                                    <x-portal::dropdown-menu.item :href="route('admin.sustainability-tabs.items.index', ['tabId' => $row->ulid, 'category' => $category])" as="a" >
                                        Items
                                    </x-portal::dropdown-menu.item>
                                    <x-portal::dropdown-menu.item :href="route('admin.sustainability-tabs.edit', ['tabId' => $row->ulid, 'category' => $category])" as="a" >
                                        Edit
                                    </x-portal::dropdown-menu.item>
                                    <x-dropdown-menu.item variant="danger" x-on:click="alertDialog='dialog-form-delete-popup';menuOpen=false" onclick="showPopupDelete('{{ route('admin.sustainability-tabs.destroy', ['tabId' => $row->ulid, 'category' => $category]) }}')">
                                        Delete
            ...
            document.querySelectorAll('.sortable').forEach(table => {
                new Sortable(table, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch("{{ route('admin.sustainability-tabs.sort', ['category' => $category]) }}", {
                            method: "POST",
                            headers: {
