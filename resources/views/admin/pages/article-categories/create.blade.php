...
@section('content')
    <div class="w-[500px] space-y-6">
        ...
        <form method="POST" action="{{ route('admin.article-categories.store') }}" class="flex flex-col gap-4">
            @csrf
            ...
            >
                ...
                <option value="0" {{ old('is_sustainability') == 0 ? 'selected' : '' }}>No</option>
                <option value="1" {{ old('is_sustainability') == 1 ? 'selected' : '' }}>Yes</option>
            </x-portal::form.select>
