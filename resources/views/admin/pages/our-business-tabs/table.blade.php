...
@section('content')
    <x-layouts.search :route-add="route('admin.page-management.our-business-tabs.create', ['id' => $businessId])" :show-add="true" />
    <div x-data="{ alertDialog: '' }">
        <x-portal::table>
            <thead>
                <x-portal::table.row class="!text-neutral-800">
                    <th></th>
                    <x-portal::table.head>Title EN</x-portal::table.head>
                    <x-portal::table.head>Title ID</x-portal::table.head>
                        ...
                        <x-portal::table.cell class="font-medium text-right">
                            <x-portal::dropdown-menu class="flex justify-end">
                                ...
                                <x-portal::dropdown-menu.content class="w-fit" align="end">
                                    <x-portal::dropdown-menu.item :href="route('admin.page-management.our-business-tabs.contents.index', ['ourBusinessTab' => $row->ulid, 'id' => $businessId])" as="a" >
                                        Contents
                                    </x-portal::dropdown-menu.item>
                                    <x-portal::dropdown-menu.item :href="route('admin.page-management.our-business-tabs.edit', ['ourBusinessTab' => $row->ulid, 'id' => $businessId])" as="a" >
                                        Edit
                                    </x-portal::dropdown-menu.item>
                                    <x-dropdown-menu.item variant="danger" x-on:click="alertDialog='dialog-form-delete-popup';menuOpen=false" onclick="showPopupDelete('{{ route('admin.page-management.our-business-tabs.destroy', ['ourBusinessTab' => $row->ulid, 'id' => $businessId]) }}')">
                                        Delete
            ...
            document.querySelectorAll('.sortable').forEach(table => {
                new Sortable(table, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch("{{ route('admin.page-management.our-business-tabs.sort', ['id' => $businessId]) }}", {
                            method: "POST",
                            headers: {
