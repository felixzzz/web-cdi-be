...
@section('content')
    <x-layouts.search :route-add="route('admin.sustainability-contents.create', ['category' => $category])" :show-add="true" />
                        ...
                        <x-portal::table.cell class="font-medium text-right">
                            <x-portal::dropdown-menu class="flex justify-end">
                                ...
                                <x-portal::dropdown-menu.content class="w-fit" align="end">
                                    <x-portal::dropdown-menu.item :href="route('admin.sustainability-contents.edit', ['tabId' => $row->ulid, 'category' => $category])" as="a" >
                                        Edit
                                    </x-portal::dropdown-menu.item>
                                    <x-dropdown-menu.item variant="danger" x-on:click="alertDialog='dialog-form-delete-popup';menuOpen=false" onclick="showPopupDelete('{{ route('admin.sustainability-contents.destroy', ['tabId' => $row->ulid, 'category' => $category]) }}')">
                                        Delete
            ...
            document.querySelectorAll('.sortable').forEach(table => {
                new Sortable(table, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch("{{ route('admin.sustainability-contents.sort', ['category' => $category]) }}", {
                            method: "POST",
                            headers: {
