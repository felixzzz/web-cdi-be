...
@section('content')
    <x-layouts.search :route-add="route('admin.our-histories.create')" :show-add="true" />
                        ...
                        <x-portal::table.cell class="font-medium text-right">
                            <x-portal::dropdown-menu class="flex justify-end">
                                ...
                                <x-portal::dropdown-menu.content class="w-fit" align="end">
                                    <x-portal::dropdown-menu.item :href="route('admin.our-histories.edit', $row->ulid)" as="a" >
                                        Edit
                                    </x-portal::dropdown-menu.item>
                                    <x-dropdown-menu.item variant="danger" x-on:click="alertDialog='dialog-form-delete-popup';menuOpen=false" onclick="showPopupDelete('{{ route('admin.our-histories.destroy', $row->ulid) }}')">
                                        Delete
            ...
            document.querySelectorAll('.sortable').forEach(table => {
                new Sortable(table, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch("{{ route('admin.our-histories.sort') }}", {
                            method: "POST",
                            headers: {
