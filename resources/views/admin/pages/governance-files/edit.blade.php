...
@section('content')
    <div class="w-[500px] space-y-6">
        ...
        <form method="POST" action="{{ route('admin.page-management.governance-files.update', ['governance_file' => $data->ulid, 'type' => $type ]) }}" class="flex flex-col gap-4" enctype="multipart/form-data">
            @csrf
            @method("PUT")

            <x-portal::form.select
                name="show_on_governance"
                label="Show on Governance Page"
                description=""
            ...
            >
                ...
                <option value="0" {{ $data->show_on_governance == 0 ? 'selected' : '' }}>No</option>
                <option value="1" {{ $data->show_on_governance == 1 ? 'selected' : '' }}>Yes</option>
            </x-portal::form.select>

            <x-portal::form.input label="Unique Key" placeholder="Unique Key" name="unique_key" :value="$data->unique_key" type="text" readonly />
