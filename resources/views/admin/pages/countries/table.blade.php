...
@section('content')
    <x-layouts.search :route-add="route('admin.countries.create')" :show-add="true" />
        ...
        <x-portal::table>
            <thead>
                <x-portal::table.row class="!text-neutral-800">
                    ...
                    <x-portal::table.head sortable key="code">Code</x-portal::table.head>
            ...
            <tbody class="divide-y divide-border">
                @forelse($data as $row)
                    <x-portal::table.row>
                        ...
                        <x-portal::table.cell class="font-medium">{{ $row->code }}</x-portal::table.cell>
                        <x-portal::table.cell class="font-medium text-right">
                            <x-portal::dropdown-menu class="flex justify-end">
                                ...
                                <x-portal::dropdown-menu.content class="w-fit" align="end">
                                    <x-portal::dropdown-menu.item :href="route('admin.countries.edit', $row->ulid)" as="a" >
                                        Edit
                                    </x-portal::dropdown-menu.item>
                                    <x-dropdown-menu.item variant="danger" x-on:click="alertDialog='dialog-form-delete-popup';menuOpen=false" onclick="showPopupDelete('{{ route('admin.countries.destroy', $row->ulid) }}')">
                                        Delete
